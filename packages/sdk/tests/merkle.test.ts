...
// Now using Poseidon hash (ZK-friendly)
...
describe("Merkle Root & Proofs", () => {
	...
	it("duplicates odd leaf", async () => {
		...
		const rootOdd = await merkleRoot([l1, l2, l3]);
		// Explicitly duplicate last leaf to simulate manual duplication
		const rootManual = await merkleRoot([l1, l2, l3, l3]);
	...
	it("builds and verifies inclusion proof (even count)", async () => {
		...
		const { root, proof } = await createMerkleProof(leaves, targetIndex);
		const leaf = leaves[targetIndex];
		if (!leaf) throw new Error("Test leaf missing");
		const ok = await verifyMerkleProof(leaf, root as Hex, proof);
		expect(ok).toBeTrue();
		console.log(
	...
	it("fails verification when proof is tampered", async () => {
		...
		const leaf = leaves[targetIndex];
		if (!leaf) throw new Error("Test leaf missing");
		const ok = await verifyMerkleProof(leaf, root as Hex, bad);
		expect(ok).toBeFalse();
