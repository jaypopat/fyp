...
import { merkleRoot, verifyMerkleProof, createMerkleProof } from '../merkle';
...
describe('Merkle Root & Proofs', () => {
  ...
  it('builds and verifies inclusion proof (even count)', async () => {
    ...
    const { root, proof } = await createMerkleProof(leaves, targetIndex, 'SHA-256');
    const leaf = leaves[targetIndex];
    if (!leaf) throw new Error('Test leaf missing');
    const ok = await verifyMerkleProof(leaf, root as `0x${string}`, proof, 'SHA-256');
    expect(ok).toBeTrue();
    console.log(`[test] inclusion even ok=${ok} root=${root} proofSteps=${proof.length}`);
  });
  ...
  it('builds and verifies inclusion proof (odd count with duplication)', async () => {
    ...
    const ok = await verifyMerkleProof(leaf, root as `0x${string}`, proof, 'SHA-256');
    expect(ok).toBeTrue();
    console.log(`[test] inclusion odd ok=${ok} root=${root} proofSteps=${proof.length}`);
  });
  ...
  it('fails verification when proof is tampered', async () => {
    ...
    const first = bad[0];
    if (!first || !first.sibling) throw new Error('First proof element missing');
    const sibHex = first.sibling.startsWith('0x') ? first.sibling.slice(2) : first.sibling;
    const flipped = sibHex.slice(0, 63) + (sibHex[63] === '0' ? '1' : '0');
    first.sibling = `0x${flipped}`;
    const leaf = leaves[targetIndex];
    if (!leaf) throw new Error('Test leaf missing');
    const ok = await verifyMerkleProof(leaf, root as `0x${string}`, bad, 'SHA-256');
    expect(ok).toBeFalse();
