...
export function assertHex32(value: string, label: string): asserts value is Hex32 {
    ...
    assert(value.length === 66, `${label} must be 0x + 64 hex chars (got length ${value.length})`);
}

export interface PathsFile { dataset: string; weights: string; }
export interface SchemaFile {
    cryptoAlgo: CommitOptions["schema"]["cryptoAlgo"];
    encodingSchema: CommitOptions["schema"]["encodingSchema"];
}
export interface CommitmentsFile { datasetMerkleRoot: Hex32; weightsHash: Hex32; }
export interface MetadataFile { name: string; description: string; creator?: string; version?: string; }
export interface MetaFile { version: number; createdAt: number; }
...
export function parseCommitmentsFile(data: unknown): CommitmentsFile {
    ...
    return { datasetMerkleRoot: d.datasetMerkleRoot as Hex32, weightsHash: d.weightsHash as Hex32 };
}
...
export function parseSchemaFile(data: unknown): SchemaFile {
    ...
    const d = data as Record<string, unknown>;
    assert(d.cryptoAlgo === 'SHA-256' || d.cryptoAlgo === 'BLAKE2b', 'schema.cryptoAlgo invalid');
    assert(d.encodingSchema === 'JSON' || d.encodingSchema === 'MSGPACK', 'schema.encodingSchema invalid');
    return { cryptoAlgo: d.cryptoAlgo as 'SHA-256' | 'BLAKE2b', encodingSchema: d.encodingSchema as 'JSON' | 'MSGPACK' };
}
