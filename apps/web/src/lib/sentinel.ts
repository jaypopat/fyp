import { hashBytes } from "@zkfair/sdk/hash";
...
/**
 * Compute leaf hash from receipt data
 */
function computeLeafHash(receipt: SentinelReceipt): string {
	const f32 = (n: number) => new Float32Array([n])[0];
	...
	const tuple = [
		...
		receipt.modelId,
		receipt.features,
		receipt.sensitiveAttr,
		f32(receipt.prediction),
		receipt.timestamp,
	];

	const encoded = new TextEncoder().encode(JSON.stringify(tuple));
	return hashBytes(encoded).toLowerCase();
}
