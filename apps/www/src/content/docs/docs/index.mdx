---
title: ZKFair
description: Introduction to ZKFair.
---
...
[ZKFair](/) is a platform for model providers to prove model fairness using Zero Knowledge Proofs in a trustless manner.

- **Privacy-Preserving**: You can prove fairness without exposing your internal dataset to third party auditing services or end users
- **Self-hosted**: It runs entirely on your infrastructure, with the only barrier to integration being installing the itmac package for secure communication between the model provider and the end user.
- **Standards-based**: It implements the OATH protocol and is based on ZKP standards.
- **Zero-Knowledge Proofs** : It uses zero knowledge proofs to prove model fairness without revealing private information.
...
If you just want to get started as fast as possible you can run the project using `bun run dev` and visit `http://localhost:3000/`.
